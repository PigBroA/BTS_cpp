...
torch::Tensor Reduction1x1Impl::forward(torch::Tensor net) {
    ...
    if(!this->is_final) {
        torch::Tensor theta = this->sigmoid(net.index({torch::indexing::Slice(), 0, torch::indexing::Slice(), torch::indexing::Slice()}))*CV_PI/3;
        torch::Tensor phi = this->sigmoid(net.index({torch::indexing::Slice(), 1, torch::indexing::Slice(), torch::indexing::Slice()}))*CV_PI*2;
